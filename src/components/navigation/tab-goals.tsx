...
export const TabGoals: React.FC<TabGoalsProps> = ({
  ...
  useEffect(() => {
    if (
      userContext?.userType === "supporter" ||
      userContext?.userType === "hybrid" ||
  ...
  // Load user's EF data on mount
  useEffect(() => {
    const loadUserEfData = async () => {
      try {
        const profile = await database.getProfile();
        if (profile?.ef_selected_pillars && profile.ef_selected_pillars.length > 0) {
          setUserEfPillars(profile.ef_selected_pillars);
          setHasEfData(true);
        } else {
          setHasEfData(false);
        }
      } catch (error) {
        console.error("Error loading EF data:", error);
      }
    };
    loadUserEfData();
  }, []);
  ...
  const renderCurrentView = () => {
    ...
    if (showQuickFlow) {
      return (
        <QuickGoalFlow
          efSelectedPillars={userEfPillars}
          onComplete={(goalData) => {
            setShowQuickFlow(false);
            handleWizardGoalCreated(goalData);
          }}
          onCancel={() => {
            setShowQuickFlow(false);
            onWizardStateChange?.(false);
    ...
    switch (currentView) {
      ...
      case "summary":
        return aiGoal ? (
          <GoalSummary goal={aiGoal} onBack={() => setCurrentView("list")} onComplete={() => setCurrentView("list")} />
        ) : (
          <GoalsList onNavigate={handleNavigate} refreshTrigger={refreshTrigger} />
        );
      case "wizard":
  ...
  const isWizardView =
    ...
    currentView === "create-wizard-supporter" ||
    currentView === "supporter-wizard" ||
    currentView === "wizard" ||
    showQuickFlow;
        ...
        <div className="p-4">
          <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as "own" | "individual")}>
            <TabsList className="inline-flex w-auto mx-auto">
              ...
              <TabsTrigger value="individual">{supportedIndividual.name}'s Goals</TabsTrigger>
            </TabsList>

            <TabsContent value="own" className="mt-0">
              <div key="own-goals">{renderCurrentView()}</div>
            </TabsContent>

            <TabsContent value="individual" className="mt-0">
              <div key="individual-goals">{renderCurrentView()}</div>
            </TabsContent>
          </Tabs>
        </div>
      </div>
  ...
  return (
    <div className="min-h-[100dvh] bg-background" style={{ paddingTop: "calc(env(safe-area-inset-top, 0px) + 4rem)" }}>
      ...
      {!isWizardView && (
        <div
          className="fixed left-0 right-0 z-40 px-4 pb-4 pt-4 bg-card"
