...
function Calendar({
  ...
  return <DayPicker month={month} onMonthChange={setMonth} showOutsideDays={showOutsideDays} className={cn("pointer-events-auto", className)} classNames={{
    ...
    head_row: "flex mb-4",
    head_cell: "text-muted-foreground rounded-md w-[42px] font-normal text-sm",
    row: "flex w-full mt-3",
    cell: "h-[42px] w-[42px] text-center text-xs p-0 relative [&:has([aria-selected].day-range-end)]:rounded-r-md [&:has([aria-selected].day-outside)]:bg-accent/50 [&:has([aria-selected])]:bg-accent first:[&:has([aria-selected])]:rounded-l-md last:[&:has([aria-selected])]:rounded-r-md focus-within:relative focus-within:z-20",
    day: cn(buttonVariants({
      variant: "ghost"
    }), "h-[38px] w-[38px] p-0 font-normal aria-selected:opacity-100 text-sm"),
