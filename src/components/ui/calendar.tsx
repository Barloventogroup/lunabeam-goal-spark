...
function CustomCaption({ displayMonth, showYearPicker }: CaptionProps & { showYearPicker?: boolean }) {
  ...
  return (
    <div className="flex justify-center pt-1 pb-2 gap-2">
      <Select
        value={month.toString()}
        onValueChange={(value) => {
          const newDate = new Date(year, parseInt(value));
          const event = new CustomEvent('monthYearChange', { detail: newDate });
  ...
  return (
    <DayPicker
      ...
      classNames={{
        ...
        head_row: "flex",
        head_cell:
          "text-muted-foreground rounded-md w-6 sm:w-8 font-normal text-[0.65rem] sm:text-[0.75rem]",
        row: "flex w-full mt-1 sm:mt-2",
        cell: "h-6 w-6 sm:h-8 sm:w-8 text-center text-[0.65rem] sm:text-xs p-0 relative [&:has([aria-selected].day-range-end)]:rounded-r-md [&:has([aria-selected].day-outside)]:bg-accent/50 [&:has([aria-selected])]:bg-accent first:[&:has([aria-selected])]:rounded-l-md last:[&:has([aria-selected])]:rounded-r-md focus-within:relative focus-within:z-20",
        day: cn(
          buttonVariants({ variant: "ghost" }),
          "h-6 w-6 sm:h-8 sm:w-8 p-0 text-[0.65rem] sm:text-xs font-normal aria-selected:opacity-100"
        ),
