...
const GoalFactorSummary = lazy(() => import('./goal-factor-summary').then(m => ({
  default: m.GoalFactorSummary
})));
const GoalCalendarView = lazy(() => import('./goal-calendar-view').then(m => ({
  default: m.GoalCalendarView
})));
...
export const GoalDetailV2: React.FC<GoalDetailV2Props> = ({
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`steps-updates-${goalId}`).on('postgres_changes', {
      event: '*',
      schema: 'public',
      table: 'steps',
  ...
  useEffect(() => {
    ...
    const isPendingStalled = status === 'pending' && startedAt && Date.now() - new Date(startedAt).getTime() > 5 * 60 * 1000;
  ...
  const progress = useMemo(() => {
    ...
    const actionableSteps = steps.filter(s => (!s.type || s.type === 'action') && !s.hidden && s.status !== 'skipped');
  ...
  const triggerDailyGeneration = async () => {
    ...
    const {
      dailyStepsGenerator
    } = await import('@/services/dailyStepsGenerator');
    dailyStepsGenerator.generateNextDaySteps(goal.id).then(result => {
  ...
  const generatePMSteps = async () => {
    ...
    try {
      ...
      const barriersText = Array.isArray(pmMetadata.barriers) ? pmMetadata.barriers.map((b: any) => typeof b === 'string' ? b : b.text || '').join('; ') : pmMetadata.barriers?.text || pmMetadata.barriers || '';
      ...
      const pmPayload = {
        ...
        prerequisites: pmMetadata.prerequisites || {
          hasEverything: true
        },
      ...
      const {
        data,
        error
      } = await supabase.functions.invoke('pm-microsteps-scaffold', {
        body: pmPayload
      });
      if (error) {
    ...
    try {
      ...
      if (dueEnd) {
        ...
        if (baseStartDate > dueEnd) {
          throw new Error(`Start date (${format(baseStartDate, 'MMM d, yyyy')}) is after goal end date (${format(dueEnd, 'MMM d, yyyy')}). Please adjust your dates.`);
        }
      }
      ...
      if (isHabitGoal) {
        ...
        if (selectedDayNumbers.length > 0) {
          ...
          const daysBetween = goalEndDate ? Math.ceil((goalEndDate.getTime() - baseStartDate.getTime()) / (1000 * 60 * 60 * 24)) : null;
          ...
          if (occurrenceDates.length === 0) {
            ...
            clearTimeout(timeoutId);
            throw new Error(`No occurrences found for selected days (${selectedDayNames}) between ${format(baseStartDate, 'MMM d, yyyy')} and ${goalEndDate ? format(goalEndDate, 'MMM d, yyyy') : 'end date'}. Please check your schedule.`);
          }
        } else {
      ...
      if (goalDueDateEnd && lastOccurrenceDate > goalDueDateEnd) {
        ...
        if (trimmedDates.length === 0) {
          ...
          throw new Error(`No ${selectedDayNames} days occur between ${format(baseStartDate, 'MMM d, yyyy')} and ${format(goalDueDateEnd, 'MMM d, yyyy')}. Please extend your end date or adjust selected days.`);
        }
      ...
      if (isHabitGoal && occurrenceDates.length > 1) {
        await supabase.from('goals').update({
          metadata: {
            ...
            lastGeneratedOccurrenceIndex: -1,
            // Will be 0 after first day
      ...
      for (let dayIndex = 0; dayIndex < daysToGenerate; dayIndex++) {
        ...
        try {
          ...
          for (let i = 0; i < individualSteps.length; i++) {
            ...
            const isCompletionStep = microStep.title.toLowerCase().includes('complete:') || microStep.title.toLowerCase().includes('help complete:');
            ...
            if (i > 0 && savedIndividualSteps[i - 1]) {
              await supabase.from('steps').update({
                dependency_step_ids: [savedIndividualSteps[i - 1].id]
              }).eq('id', step.id);
            }
          }
          ...
          if (wizardData.primarySupporterRole === 'hands_on_helper') {
            ...
            for (let i = 0; i < supporterSteps.length; i++) {
              ...
              const {
                ...
                step_type: 'action',
                is_required: false,
                // Supporter steps are guidelines, not blocking requirements
      ...
      toast({
        ...
        description: isHabitGoal && occurrenceDates.length > 1 ? `✅ Day 1 steps generated! The next ${occurrenceDates.length - 1} ${occurrenceDates.length - 1 === 1 ? 'day' : 'days'} will be generated automatically as you complete each day.` : "Your personalized steps have been generated."
      });
    } catch (error: any) {
  ...
  return <div className="min-h-[100dvh] bg-background">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full mt-3">
        <TabsList className="w-full p-0 px-4 items-stretch overflow-x-auto overflow-y-hidden inline-flex scrollbar-hide justify-start">
          ...
          <TabsTrigger value="steps" className="h-full min-h-0 py-2 leading-none flex items-center justify-center gap-2 shadow-none data-[state=active]:shadow-none flex-shrink-0 min-w-[100px]">
            Steps
          ...
          <TabsTrigger value="calendar" className="h-full min-h-0 py-2 leading-none flex items-center justify-center gap-2 shadow-none data-[state=active]:shadow-none flex-shrink-0 min-w-[100px]">
            Results
          </TabsTrigger>
          {isViewerSupporter && steps.filter(s => s.is_supporter_step).length > 0 && <TabsTrigger value="supporter" className="h-full min-h-0 py-2 leading-none flex items-center justify-center gap-2 shadow-none data-[state=active]:shadow-none flex-shrink-0 min-w-[140px]">
              Supporter Setup
