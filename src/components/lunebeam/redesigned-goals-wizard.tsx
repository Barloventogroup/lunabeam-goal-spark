  ...
  const nextStep = async () => {
    ...
    if (currentStep === 7 && currentStep + 1 === 8) {
      ...
      try {
        ...
        // Generate coach steps ONLY if primary supporter is hands-on helper with delivery method
        // Note: Coach steps now generated by same microsteps-scaffold with flow='supporter'
        if (data.primarySupporterRole === 'hands_on_helper' && data.supporterDeliveryMethod) {
          // Coach steps are now part of the main microSteps generation above with flow='supporter'
          // The supporter flow in microsteps-scaffold provides natural, context-aware guidance
          setGeneratedCoachSteps(microSteps); // Use the supporter-flow steps generated above
        }
      } catch (error) {
        ...
        {/* Delivery Method - only for hands-on helper */}
         {data.primarySupporterRole === 'hands_on_helper' && <div className="space-y-4 mt-6 pt-6 border-t">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Choose the type of hands-on support
              </p>
            </div>
