...
BEGIN
  ...
  -- Check if current user is admin for this user or the user themselves
  IF _current_user != user_id_to_delete AND NOT EXISTS (
    SELECT 1 FROM supporters 
    WHERE individual_id = user_id_to_delete 
