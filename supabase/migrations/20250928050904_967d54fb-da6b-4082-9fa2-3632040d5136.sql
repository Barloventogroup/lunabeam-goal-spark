-- Fix create_supporter_invite_secure function to allow inviting supporters for oneself
-- The issue is that it currently blocks when individual_id = inviter_id, 
-- but users should be able to invite supporters for themselves.
-- Instead, we should only block if inviting their own email address.
