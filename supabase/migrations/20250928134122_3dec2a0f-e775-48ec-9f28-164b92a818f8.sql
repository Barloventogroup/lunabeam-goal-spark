-- Fix ambiguous column references in approval functions

-- Recreate approve_supporter_request_by_email with fully qualified column references
...
BEGIN
  ...
  -- Find the pending admin approval request by email (fully qualify all columns)
  ...
  BEGIN
    DELETE FROM public.supporter_invites WHERE public.supporter_invites.id = _request_record.id;
...
-- Recreate approve_supporter_request_secure with fully qualified column references  
...
BEGIN
  ...
  SELECT * INTO _request_record 
  FROM public.supporter_invites 
  WHERE public.supporter_invites.id = p_request_id AND public.supporter_invites.status = 'pending_admin_approval';
  ...
  IF NOT EXISTS (
    ...
    WHERE public.supporters.individual_id = _request_record.individual_id 
      AND public.supporters.supporter_id = _current_user 
      AND public.supporters.is_admin = true
  ) THEN
    RAISE EXCEPTION 'Unauthorized: Must be admin for this individual';
  END IF;
