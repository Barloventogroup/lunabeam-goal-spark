...
-- Create function to calculate points based on category and step type
...
BEGIN
  ...
  IF NEW.status = 'done' AND (OLD.status IS NULL OR OLD.status != 'done') THEN
    ...
    SELECT title, domain INTO _goal_record
    FROM goals 
    ...
    -- Get the goal owner (user_id)
    SELECT owner_id INTO _goal_record.owner_id 
    FROM goals 
    WHERE id = NEW.goal_id;
...
-- Create trigger to award points when steps are completed
CREATE TRIGGER award_points_on_step_completion
  AFTER UPDATE ON steps
  FOR EACH ROW
  EXECUTE FUNCTION award_step_points();