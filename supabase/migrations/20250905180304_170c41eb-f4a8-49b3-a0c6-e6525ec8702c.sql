-- Create enum types for roles and permission levels (if they don't exist)
DO $$ BEGIN
...
-- Add account status and metadata to profiles (only if columns don't exist)
DO $$ BEGIN
...
CREATE TABLE IF NOT EXISTS public.supporter_invites (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
...
DROP POLICY IF EXISTS "Individuals can view their supporters" ON public.supporters;
DROP POLICY IF EXISTS "Supporters can view their relationships" ON public.supporters;
DROP POLICY IF EXISTS "Individuals can manage their supporters" ON public.supporters;
DROP POLICY IF EXISTS "Provisioners can update during setup" ON public.supporters;
...
-- Create trigger to update timestamps (only if not exists)
...
CREATE INDEX IF NOT EXISTS idx_account_claims_token ON public.account_claims(claim_token);
CREATE INDEX IF NOT EXISTS idx_supporter_invites_token ON public.supporter_invites(invite_token);