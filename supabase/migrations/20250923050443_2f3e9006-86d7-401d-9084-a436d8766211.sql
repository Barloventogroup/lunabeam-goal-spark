-- Fix the provision_individual_direct function to properly handle UUID generation and auth.users creation
...
BEGIN
  ...
  -- Generate a unique ID that doesn't exist in auth.users
  LOOP
    _new_individual_id := gen_random_uuid();
    _attempt_count := _attempt_count + 1;
    
    -- Check if this ID already exists in auth.users (not just profiles)
    IF NOT EXISTS (SELECT 1 FROM auth.users WHERE id = _new_individual_id) THEN
  ...
  -- Generate other values with the confirmed unique ID
  ...
  -- Create the auth.users record first
