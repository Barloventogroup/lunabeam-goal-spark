-- Fix the ambiguous column reference error in provision_individual_direct
CREATE OR REPLACE FUNCTION public.provision_individual_direct(p_first_name text, p_strengths text[] DEFAULT ARRAY[]::text[], p_interests text[] DEFAULT ARRAY[]::text[], p_comm_pref text DEFAULT 'text'::text)
...
BEGIN
  ...
  -- Create supporter relationship using exception-safe approach
  BEGIN
  ...
  -- Use RETURN QUERY SELECT to avoid any ambiguous references
