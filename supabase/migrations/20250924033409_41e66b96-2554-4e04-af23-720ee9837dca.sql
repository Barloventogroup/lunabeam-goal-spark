-- Drop and recreate assign_email_and_invite to fix return type
DROP FUNCTION IF EXISTS public.assign_email_and_invite(uuid, text, text);
...
BEGIN
  ...
  UPDATE public.account_claims
     SET invitee_email = _real_email,
         ...
         claim_token = COALESCE(claim_token, _claim_token),
         status = 'pending'
   WHERE individual_id = p_individual_id
