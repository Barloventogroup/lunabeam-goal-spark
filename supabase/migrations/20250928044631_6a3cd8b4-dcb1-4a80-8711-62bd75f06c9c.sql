-- Patch ambiguous status references by qualifying table columns inside functions that RETURN TABLE(..., status invite_status)

-- approve_supporter_request_secure: qualify status in WHERE clause
...
-- approve_supporter_request_by_email: qualify status in WHERE clause
