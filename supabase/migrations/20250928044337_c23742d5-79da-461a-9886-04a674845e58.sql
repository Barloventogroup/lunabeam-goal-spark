-- Fix ambiguous column reference in approval functions by aliasing function results

-- Update approve_supporter_request_secure to use aliased function result
...
BEGIN
  ...
  -- Load the pending admin approval request
  SELECT * INTO _request_record
  FROM public.supporter_invites 
  WHERE supporter_invites.id = p_request_id AND status = 'pending_admin_approval';
  ...
  -- Fix: Alias the function result to avoid ambiguous column reference
...
-- Update approve_supporter_request_by_email to use aliased function result  
...
BEGIN
  ...
  -- Find the pending admin approval request by email
  SELECT * INTO _request_record
  FROM public.supporter_invites 
