-- Harden admin_delete_user: temporarily disable supporters triggers to bypass last-admin guard during full deletions
...
BEGIN
  -- Disable supporters triggers to avoid last-admin constraint while performing full cleanup
  PERFORM 1;
  BEGIN
    EXECUTE 'ALTER TABLE public.supporters DISABLE TRIGGER ALL';
    _triggers_disabled := true;
  EXCEPTION WHEN others THEN
    -- If we cannot disable triggers, continue and attempt promotion fallback
    _triggers_disabled := false;
  END;

  -- Fallback: if triggers not disabled, try to promote another supporter where needed
